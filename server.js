...
// This is the memory — it stores each conversation by phone number
const conversations = {};
...
app.post('/webhook', async (req, res) => {
  ...
  // Create a new conversation history for this number if one does not exist
  ...
  // Keep only the last 20 messages to avoid it getting too long
  if (conversations[From].length > 20) {
    conversations[From] = conversations[From].slice(-20);
  }

  try {
    // Send the full conversation history to Claude every time
    ...
    // Add the reply to the conversation history so Claude remembers it next time
    ...
    const twiml = new twilio.twiml.MessagingResponse();
    twiml.message("Sorry, we are having a technical issue right now. Please call us on business hours and we will be happy to help!");
    res.type('text/xml').send(twiml.toString());
  }
});

// This is just a health check so you can confirm the bot is running
app.get('/', (req, res) => {
...
5. Click **"Commit changes"** again to confirm
